...
import { ShieldCheck, CheckCircle2, XCircle, RotateCcw, Lock, GitPullRequest, GitMerge, ExternalLink } from 'lucide-react';
...
export default function ApprovalResolution({
  ...
  const getStatusBadge = () => {
    if (status === 'merged') {
      return <span className="text-[#3fb950] font-bold bg-[#113216] border border-[#238636] px-2.5 py-1 rounded">Merged into main</span>;
    }
    if (status === 'discarded') {
      return <span className="text-[#f85149] font-bold bg-[#3c1e1e] border border-[#7d2727] px-2.5 py-1 rounded">Remediation discarded</span>;
    }
    if (status === 'reverted') {
      return <span className="text-[#d29922] font-bold bg-[#342b10] border border-[#9e6a03] px-2.5 py-1 rounded">Revert created</span>;
    }
    return <span className="text-[#d29922] font-bold bg-[#342b10] border border-[#9e6a03] px-2.5 py-1 rounded">Awaiting Approval</span>;
  };
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3 pt-4 border-t border-[#30363d]">
          ...
          {status !== 'discarded' && (
            ...
            >
              ...
              {acting ? 'Executing Rollback...' : 'Rollback'}
            </button>
          )}
